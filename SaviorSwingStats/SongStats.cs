...
namespace SaviorSwingStats
{
    struct SongStats
    {
        ...
        public SongStats(int number, bool goodCut, float time, int type, string direction, int column, int row, float cutCenterOff, float cutTimeOff, int comboX, float before, float after)
        {
            Number = number;
            GoodCut = goodCut;
            Time = time;
